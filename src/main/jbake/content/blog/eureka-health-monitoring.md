...
date=2020-05-21
type=post
tags=spring-cloud, eureka, microservices
status=published
subtitle=A story about the mysteries of Netflix Eureka health monitoring
description=Understanding Eureka health checks - learnt vs taught monitoring, REST operations, self-diagnosis, health endpoints, and accuracy caveats.
~~~~~~
...
Health checks are performed in order to identify and evict unhealthy (i.e. down, unreachable) microservices from the Eureka server registry. However, Eureka servers never send keep-alive requests to their registered clients (as opposed to some [Traffic Managers](https://community.pulsesecure.net/t5/Pulse-Secure-vADC/Feature-Brief-Health-Monitoring-in-Stingray-Traffic-Manager/ta-p/28516)), instead, Eureka clients send heartbeats to Eureka servers.

<figure>
<img src="/images/blog/eureka-health-monitoring/eureka-clients-sending-heartbeats.png" alt="Eureka clients sending heartbeats to server" style="width:70%">
<figcaption>Eureka clients sending heartbeats to server</figcaption>
</figure>
...
<figure>
<img src="/images/blog/eureka-health-monitoring/asgard-is-overriding-the-status.png" alt="Asgard is overriding the status which has been published by instance heartbeat">
<figcaption>Asgard is overriding the status which has been published by instance heartbeat</figcaption>
</figure>
...
The default value of the `health-check-url-path` is `/health` which is the spring-boot's default health actuator endpoint that will be ignored if a `health-check-url` property is set. Ideally you should configure these properties if you implement a custom health endpoint or change the properties impacting the default health endpoint path. i.e.

- If you change the default health endpoint path

```properties
endpoints.health.path=/new-health
...
<figure>
<img src="/images/blog/eureka-health-monitoring/ribbon-making-load-balancing-decisions.png" alt="Ribbon relies on the status attribute available in the registry in order to make load balancing decisions">
<figcaption>Ribbon relies on the status attribute available in the registry in order to make load balancing decisions</figcaption>
</figure>
...
<figure>
<img src="/images/blog/eureka-health-monitoring/asgard-invoking-health-check.png" alt="Asgard is invoking the healthCheckUrl until the first instance becomes UP">
<figcaption>Asgard is invoking the healthCheckUrl until the first instance becomes UP</figcaption>
</figure>
